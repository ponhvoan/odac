...
tiny_imagenet = input("Tiny imagenet (y/n)? ")

if tiny_imagenet=='y':
    directory ='/mnt/HDPH/datasets/tiny-imagenet-200'
    file_path = directory + '/wnids.txt'
    folders = []
    ...
    directory = '/home/hirokiwaida/data/imagenet/'
...
if __name__ == '__main__':
    ...
    for i, folder_name in enumerate(folders):
        ...
        for extension in IMG_EXTENSIONS:
            if tiny_imagenet=='y':
                files.extend(glob.glob(os.path.join(directory, 'train', folder_name, 'images','*' + extension)))
            else:
                files.extend(glob.glob(os.path.join(directory, 'train', folder_name,'*' + extension)))
