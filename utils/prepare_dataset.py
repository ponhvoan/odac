...
def prepare_test_data(args, ttt=False, num_sample=None):
    ...
    if args.dataset == 'cifar10':
        ...
        tesize = 10000
        if not hasattr(args, 'corruption') or args.degrade_choice == 'original':
            print('Test on the original test set')
            teset = torchvision.datasets.CIFAR10(root=args.dataroot,
                                                train=False, download=True, transform=te_transforms)
        elif args.degrade_choice in common_corruptions:
            print('Test on %s level %d' %(args.degrade_choice, args.level))
            teset_raw = np.load(args.dataroot + '/CIFAR-10-C/%s.npy' %(args.degrade_choice))
            teset_raw = teset_raw[(args.level-1)*tesize: args.level*tesize]
            teset = torchvision.datasets.CIFAR10(root=args.dataroot,
                                                train=False, download=True, transform=te_transforms)
            teset.data = teset_raw

        elif args.degrade_choice == 'cifar_new':
        ...
        elif args.degrade_choice == 'cifar_mix10':
        ...
        elif args.degrade_choice == 'cifar_mix5':
        ...
        if not hasattr(args, 'corruption') or args.degrade_choice == 'original':
            ...
            print('Test on %s level %d' %(args.degrade_choice, args.level))
            teset_raw = np.load(args.dataroot + '/CIFAR-100-C/%s.npy' %(args.degrade_choice))
            teset_raw = teset_raw[(args.level-1)*tesize: args.level*tesize]
            teset = torchvision.datasets.CIFAR100(root=args.dataroot,
                                                train=False, download=True, transform=te_transforms)
            teset.data = teset_raw
...
def prepare_train_data(args, num_sample=None):
    ...
    if args.dataset == 'cifar10':
        ...
        if hasattr(args, 'ssl') and args.ssl == 'contrastive':
            ...
            if hasattr(args, 'degrade_choice') and args.degrade_choice in common_corruptions:
                print('Contrastive on %s level %d' %(args.degrade_choice, args.level))
                tesize = 10000
                trset_raw = np.load(args.dataroot + '/CIFAR-10-C/%s.npy' %(args.degrade_choice))
                trset_raw = trset_raw[(args.level-1)*tesize: args.level*tesize]   
                trset.data = trset_raw
            elif hasattr(args, 'degrade_choice') and args.degrade_choice == 'cifar_new':
                from utils.cifar_new import CIFAR_New
            ...
            elif  hasattr(args, 'degrade_choice') and args.degrade_choice == 'cifar_mix10':
                print('Test on mix on 10 noises on level %d' %(args.level))
                trset_mix_raw = prepare_mix_corruption(args, 10, args.dataroot + '/CIFAR-10-C')
                trset.data = trset_mix_raw
            elif  hasattr(args, 'degrade_choice') and args.degrade_choice == 'cifar_mix5':
                print('Test on mix on 5 noises on level %d' %(args.level))
        ...
        if hasattr(args, 'ssl') and args.ssl == 'contrastive':
            ...
            if hasattr(args, 'degrade_choice') and args.degrade_choice in common_corruptions:
                print('Contrastive on %s level %d' %(args.degrade_choice, args.level))
                tesize = 10000
                trset_raw = np.load(args.dataroot + '/CIFAR-100-C/%s.npy' %(args.degrade_choice))
                trset_raw = trset_raw[(args.level-1)*tesize: args.level*tesize]   
                trset.data = trset_raw
            else:
                print('Contrastive on ciar10 training set')
